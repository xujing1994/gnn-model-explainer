...
from utils.io_utils import S2VGraph
...
def benchmark_task(args, graphs, writer=None, feat="node-feat"):
    #graphs = io_utils.read_graphfile(
    #    args.datadir, args.bmname
    #)
    #random.shuffle(graphs)
    ...
    evaluate(test_dataset, model, args, "Validation")
    return model, cg_data, train_dataset, val_dataset, test_dataset
...
def main():
    ...
    graphs = io_utils.read_tudataset(
        prog_args.datadir
    )
    ...
    if prog_args.bmname is not None:
        model, cg_data, training_dataset, val_dataset, test_dataset = benchmark_task(prog_args, graphs, writer=writer, feat="node-feat")
